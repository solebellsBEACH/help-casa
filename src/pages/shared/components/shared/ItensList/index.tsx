...
import {
  getCategoryImage,
  ImageKey,
} from "@/pages/shared/utils/getCategoryImage";
...
const ProductItem = ({ data }: { data: Service }) => {
  ...
  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg item-product">
      <img
        className="w-full h-24 px-1 product-image"
        src={
          getCategoryImage(data?.category as ImageKey) ||
          "/images/product-image-example.png"
        }
      />
      <div className="px-6 py-4">
        ...
        <p className="text-gray-700 text-sm">{data.serviceDescription}</p>
        <button
          className="bg-primary w-full my-2 rounded details-button"
...
function ItensList({ data, title }: IItensListProps) {
  console.log(data);
  return (
